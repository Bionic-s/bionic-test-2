...
import { CheckCircle, AlertCircle, ArrowRight, Phone, Mail, Clock, Layers, BarChart3, BookOpen } from 'lucide-react';
...
export default function ArabicContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || data.error) throw new Error(data.error?.message || 'فشل الإرسال');
      ...
      setIsSubmitting(false);
      setError(`تعذّر إرسال طلبك. حاول مرة أخرى، أو راسلنا مباشرة على ${contactEmails.general}.`);
    }
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" id="role" name="role" value={formData.role} onChange={handleChange} onBlur={() => handleBlur('role')}
                        className={inputCls('role')} placeholder="مثال: رئيس تقنية المعلومات، مدير المشتريات" />
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 mb-12">
                {[
                  ...
                  { label: 'المخططات', desc: '٢٧ معمارية مرجعية', path: '/ar/blueprints', icon: BookOpen },
                ].map((link) => (
