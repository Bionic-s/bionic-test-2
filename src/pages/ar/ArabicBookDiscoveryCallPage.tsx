...
export default function Page() {
  ...
  const validate = (): boolean => {
    ...
    else if (!validateEmail(email)) errs.email = 'يرجى إدخال بريد إلكتروني صحيح';
    if (!company.trim()) errs.company = 'مطلوب';
    if (!selectedDate) errs.date = 'يرجى اختيار التاريخ';
    if (!selectedTime) errs.time = 'يرجى اختيار الوقت';
    setFieldErrors(errs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) { setError('يرجى إكمال جميع الحقول المطلوبة.'); return; }
    ...
    try {
      ...
      if (!res.ok || data.error) throw new Error(data.error?.message || 'فشل إرسال الحجز');
      setIsSubmitting(false);
      navigate('/ar/thank-you?source=discovery');
    } catch (err: any) {
      setIsSubmitting(false);
      ...
      <div className="container mx-auto px-4 lg:px-12 max-w-4xl">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              {
                icon: Calendar,
                heading: 'تقييم الوضع الحالي',
                body: 'نستعرض منصاتكم وعمليات التكامل والقيود التي تتعاملون معها حاليًا.',
              },
              {
                icon: CheckCircle,
                heading: 'خيارات المعمارية المستهدفة',
                body: 'خيارات المعمارية عبر منظومة شركائنا — Salesforce، IBM، Google Cloud، Dell، Intel، MuleSoft، Tableau، Informatica، Platform9.',
              },
              {
                icon: Video,
                heading: 'جاهزية البيئة التشغيلية',
                body: 'الحوكمة والعمليات وما يعنيه المسار الموثوق للإطلاق الفعلي لفريقكم.',
              },
            ].map((card) => (
              <div
                key={card.heading}
        ...
        >
          <div className="bg-bg-secondary border border-white/5 rounded-2xl p-8">
            <h2 className="text-2xl font-bold text-text-primary text-center mb-8">تفاصيل الاجتماع</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <Clock className="w-6 h-6 text-[#00BFFF] mx-auto mb-3" />
                <p className="text-sm text-text-muted mb-1">المدة</p>
                <p className="text-lg font-semibold text-text-primary">٣٠ دقيقة</p>
              </div>
              <div className="text-center">
                <Video className="w-6 h-6 text-[#00BFFF] mx-auto mb-3" />
                <p className="text-sm text-text-muted mb-1">الصيغة</p>
                <p className="text-lg font-semibold text-text-primary">اجتماع مرئي (Zoom أو Teams)</p>
              </div>
              <div className="text-center">
                <MapPin className="w-6 h-6 text-[#00BFFF] mx-auto mb-3" />
                <p className="text-sm text-text-muted mb-1">التوفر</p>
                <p className="text-sm font-semibold text-text-primary leading-relaxed">
                  الأحد–الخميس<br />
                  ٨:٠٠ صباحًا – ٥:٠٠ مساءً<br />
                  <span className="text-text-muted font-normal">التوقيت العربي القياسي (UTC+3)</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <motion.section
          ...
          className="mb-20"
        >
          <h2 className="text-2xl font-bold text-text-primary text-center mb-2">احجز مكالمتك</h2>
          <p className="text-text-muted text-center mb-10 max-w-[480px] mx-auto">
            اختر التاريخ والوقت. ستُرسل إليك رسالة تأكيد بالبريد الإلكتروني مع دعوة تقويم ورابط الاجتماع.
          </p>
          ...
          {!submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-8" noValidate>
                ...
                <AnimatePresence>
                  {selectedDate && (
                    ...
                    >
                      <label className="block text-sm font-semibold text-text-primary mb-3">
                        اختر الوقت <span className="text-text-muted font-normal">(UTC+3)</span>
                      </label>
                ...
                <div className="space-y-5 pt-4 border-t border-white/5">
                  <div>
                    ...
                    <input id="bdc-name" type="text" value={name}
                      ...
                      className={inputCls('name')} placeholder="الاسم الكامل" />
                    {fieldErrors.name && (
                  ...
                  <div>
                    ...
                    <input id="bdc-company" type="text" value={company}
                      ...
                      className={inputCls('company')} placeholder="الجهة أو المؤسسة" />
                    {fieldErrors.company && (
                  ...
                  <div>
                    <label htmlFor="bdc-phone" className="block text-sm text-text-muted mb-2">
                      رقم الهاتف <span className="text-text-muted">(اختياري)</span>
                    </label>
                  ...
                  <div>
                    <label htmlFor="bdc-role" className="block text-sm text-text-muted mb-2">
                      المسمى الوظيفي <span className="text-text-muted">(اختياري)</span>
                    </label>
                    <input id="bdc-role" type="text" value={role}
                      onChange={(e) => setRole(e.target.value)}
                      className={inputCls('role')} placeholder="مثال: رئيس تقنية المعلومات، مدير المعمارية" />
                  </div>
                  <div>
                    <label htmlFor="bdc-notes" className="block text-sm text-text-muted mb-2">
                      ملاحظات <span className="text-text-muted">(اختياري)</span>
                    </label>
                    <textarea id="bdc-notes" value={notes}
                      onChange={(e) => setNotes(e.target.value)}
                      rows={4}
                      className={`${inputCls('notes')} resize-none`}
                      placeholder="أي شيء ترغب في أن نراجعه قبل المكالمة — البنية الحالية، الحالة المستهدفة، متطلبات الامتثال، إلخ."
                    />
                  </div>
                </div>
                ...
                <button type="submit" disabled={isSubmitting}
                  ...
                  {isSubmitting ? (
                    <><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" /><span>جارٍ الحجز...</span></>
                  ) : (
                    <><span>تأكيد الحجز</span><Send className="w-5 h-5" /></>
                  )}
                </button>

                <p className="text-tiny text-text-muted text-center">
                  بتأكيد الحجز، أنت توافق على تواصُلنا معك بخصوص استفسارك. لا نشارك معلوماتك مع أي طرف.
                </p>
              </form>
            </div>
          ) : (
            ...
            >
              ...
              <h2 className="text-2xl md:text-3xl font-bold mb-4">تم استلام الحجز</h2>
              <p className="text-text-muted text-lg max-w-[480px] mx-auto mb-8">
                سنرسل دعوة تقويم خلال ٢٤ ساعة إلى <span className="text-text-primary font-medium">{email}</span>.
              </p>
        ...
        >
          <div className="bg-bg-secondary border border-white/5 rounded-2xl p-8 md:p-10">
            <h2 className="text-2xl font-bold text-text-primary mb-6">كيف تستعد</h2>
